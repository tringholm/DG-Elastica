// dgstepMexPara propagates one timestep of the discrete gradient algorithm 
// on a block using a C implementation
// 
// Input:
...
// 
// Output:
// u       - updated block after one time step
...
void dgstep(double *u, double *g, double *K, int Nx, int Ny, double dt, double a, double b,
        double s, double epsilon, double tol, double *fcountout, int blocy, int blocx, int maxy);

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    // ------------------------------------------ initialize and get input
    double dt, a, b, s, epsilon, tol;
    double *u, *g, *g_b, *u_out, *K, *K_b, *fcountout, *blockSize, *location, *imageSize;
    int  i, j, Nx, Ny, blocy, blocx, maxy, globind;
    const mxArray  *blockSizeMx, *locationMx, *imageSizeMx, *u_in; 
    
    // ------------------------------------------ get block and image size
    u_in = mxGetField(u_block,0,"data");
    blockSizeMx = mxGetField(u_block,0,"blockSize");
    locationMx = mxGetField(u_block,0,"location");
    imageSizeMx = mxGetField(u_block,0,"imageSize");
       
    blockSize = mxGetData(blockSizeMx);
    location = mxGetData(locationMx);
    imageSize = mxGetData(imageSizeMx);
    
    Ny = (int)blockSize[0];
    Nx = (int)blockSize[1];
    blocy = (int)location[0];
    blocx = (int)location[1];
    maxy = (int)imageSize[0];
    ...
    *fcountout = 0;
    ...
    g_b = (double*)mxMalloc(Ny*Nx*sizeof *g_b);
    K_b = (double*)mxMalloc(Ny*Nx*sizeof *K_b);
    

    
    for (i = 0; i < Nx; i++){
        for (j = 0; j < Ny; j++){
            globind = blocy - 1 + j +(blocx - 1 +i)*maxy;
            g_b[j + i*Ny] = g[globind];
            K_b[j + i*Ny] = K[globind];
        }
    }
    
    // ------------------------------------------ take one time step
    dgstep(u, g_b, K_b, Nx, Ny, dt, a, b, s, epsilon, tol, fcountout, blocy, blocx, maxy);
    
    // ------------------------------------------ deliver output
    plhs[0] = mxCreateDoubleMatrix(Ny+4, Nx+2, mxREAL);
    u_out = mxGetPr(plhs[0]);
    for (i = 0; i < Nx+2; i++){
        for (j = 0; j < Ny+4; j++){
            u_out[j+ i*(Ny+4)] = u[j+ i*(Ny+4)];
        }
    }
    mxFree(g_b);
    mxFree(K_b);
...
void dgstep(double *u, double *g, double *K, int Nx, int Ny, double dt, double a, double b,
    ...
    double u_old, E, sqrteps, fcount;
    int i, j, k, l, ind, top, bot, globi;
    ...
    for (j = 2; j < Nx+1; j++){
        for (i = 2*top; i < Ny+4-2*bot; i++){
            Dxu[i + j*(Ny+4)] = u[i + j*(Ny+4)] - u[i + (j-1)*(Ny+4)];
        }
    }
    for (j = 1; j < Nx+1; j++){
        for (i = 2*top; i < Ny+3-2*bot; i++){
            Dyu[i + j*(Ny+3)] = u[i+1 + j*(Ny+4)] - u[i + j*(Ny+4)];
        }
    }
    ...
    if(top == 1){
        for (j = 1; j < Nx+1; j++){
            Dxu2[1 + j*(Ny+3)] = 0.5*(Dxu[2 + j*(Ny+4)] + Dxu[2 + (j+1)*(Ny+4)]);
        }
    }
    if (bot == 1){
        for (j = 1; j < Nx+1; j++){
            Dxu2[Ny + j*(Ny+3)] = 0.5*(Dxu[Ny+1 + j*(Ny+4)] + Dxu[Ny+1 + (j+1)*(Ny+4)]);
        }
    }
    
    for (j = 2; j < Nx+1; j++){
        for (i = top; i < Ny+2-bot; i++){
    ...
    fcount = 0;
    // ------------------------------------------ main loop
    for (i = 0 + 2*(1-top); i < Ny - 2*(1-bot); i++){
        globi = blocy + i - 1;
        for (j = 0; j < Nx; j++){
            E = 0;
            ...
            fzeroFastMex(u_old+0.1, u_old,  u_l,  Dxul,  Dxuc, Dxur, Dxumt, Dxuml,
                    ...
                    E, K[i + j*Ny], g[i + j*Ny], globi, j, maxy, Nx, dt,
            ...
            if (globi == 0){
                for (k = 0; k < 2; k++){
                    Dxu2[i+1 + (j+k)*(Ny+3)] = 0.5*(Dxu[i+2 + (j+k)*(Ny+4)] + Dxu[i+2 + (j+k+1)*(Ny+4)]);
                }
            }
            else if (globi == maxy-1){
                for (k = 0; k < 2; k++){
                    Dxu2[i+2 + (j+k)*(Ny+3)] = 0.5*(Dxu[i+2 + (j+k)*(Ny+4)] + Dxu[i+2 + (j+k+1)*(Ny+4)]);
                }
            }
            if (j == 0){
                for (l = 0; l < 3; l++){
