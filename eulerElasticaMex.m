function [u, energy] = eulerElasticaMex(g,K,a,b,s,options)

% eulerElasticaMex computes an Euler's elastica regularized denoised and/or 
% inpainted image using a serial MEX function for the time steps.  
%
% Input:
...
for tstep = 1:maxit
    ...
    if adapt
        gg = gradTV(u,epsilon);
        gc = gradCurvMex(u,epsilon,Nx,Ny);
        if s == 2
    ...
    %------------------------------------------ one timestep in MEX
    u = padarray(u,[1 1]);
    u = dgstepMex(u,Nx,Ny,g,dt,a,b,s,epsilon,xtol,K);
    u = u(2:end-1,2:end-1);
