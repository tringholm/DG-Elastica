...
#define s_in            prhs[9]
#define epsilon_in      prhs[10]
#define tol_in          prhs[11]
...
void dgstep(double *u, double *g, double *K, int Nx, int Ny, double dx, double dy, double dt, double a, double b,
        double alpha, double s, double epsilon, double tol, double *fcountout, int blocy, int blocx, int maxy);
...
//     g_b = 0;
    ...
    dgstep(u, g_b, K_b, Nx, Ny, dx, dy, dt, a, b, alpha, s, epsilon, tol, fcountout, blocy, blocx, maxy);
...
void dgstep(double *u, double *g, double *K, int Nx, int Ny, double dx, double dy, double dt, double a, double b,
    ...
    for (j = 2; j < Nx+1; j++){
        for (i = 2*top; i < Ny+4-2*bot; i++){
            Dxu[i + j*(Ny+4)] = (u[i + j*(Ny+4)] - u[i + (j-1)*(Ny+4)])/dx;
        }
    }
    for (j = 1; j < Nx+1; j++){
    ...
    for (j = 1; j < Nx+1; j++){
        for (i = 2*top; i < Ny+3-2*bot; i++){
            Dyu[i + j*(Ny+3)] = (u[i+1 + j*(Ny+4)] - u[i + j*(Ny+4)])/dy;
        }
    }
    for (j = 2; j < Nx+1; j++){
    ...
    for (i = 0 + 2*(1-top); i < Ny - 2*(1-bot); i++){
        ...
        for (j = 0; j < Nx; j++){
            ...
            fzero(u_old+0.1, u_old,  u_l,  Dxul,  Dxuc, Dxur, Dxumt, Dxuml,
                    ...
                    E, K[i + j*Ny], g[i + j*Ny], globi, j, maxy, Nx, dx, dy, dt,
...
//     if (isnan(out)){
//         for (Nx = 0; Nx < 100000000; Nx++){
//             printf("i: %i   j: %i  E: %f   gamma: %f  \n",i,j,E,gamma);
//         }
//     }
