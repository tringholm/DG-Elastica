...
double coordFxnMex(double u,double u_old,double *u_l_in, double *Dxul_in, double *Dxuc_in,
        ...
        double *Dx_in, double*Dy_in, double E, double K, double g, int i, int j, int Ny, int Nx, double dt, double a, double b, double s, double epsilon);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    tol = *mxGetPr(tol_in);
    fcountout = (double*)malloc(sizeof(double));
    *fcountout = 0;
    ...
    // ------------------------------------------ deliver output
    plhs[0] = mxCreateDoubleMatrix(Ny+2, Nx+2, mxREAL);
    u_out = mxGetPr(plhs[0]);
    for (i = 0; i < Nx+2; i++){
        for (j = 0; j < Ny+2; j++){
            u_out[j+ i*(Ny+2)] = u[j+ i*(Ny+2)];
        }
    }
...
void dgstep(double *u, double *g, double *K, int Nx, int Ny, double dt, double a, double b,
    ...
    double* Dxu =        (double*)mxCalloc((Ny+2)*(Nx+3), sizeof(double));
    ...
    for (j = 2; j < Nx+1; j++){
        for (i = 1; i < Ny+1; i++){
            Dxu[i + j*(Ny+2)] = u[i + j*(Ny+2)] - u[i + (j-1)*(Ny+2)];
        }
    }
    for (j = 1; j < Nx+1; j++){
        for (i = 2; i < Ny+1; i++){
            Dyu[i + j*(Ny+3)] = u[i + j*(Ny+2)] - u[i-1 + j*(Ny+2)];
        }
    }
    ...
    for (j = 1; j < Nx+1; j++){
        for (i = 2; i < Ny+1; i++){
            Dxu2[i + j*(Ny+3)] = 0.25*(Dxu[i + j*(Ny+2)] + Dxu[i-1 + j*(Ny+2)] + Dxu[i + (j+1)*(Ny+2)] + Dxu[i-1 + (j+1)*(Ny+2)]);
        }
    }
    for (j = 1; j < Nx+1; j++){
        Dxu2[1 + j*(Ny+3)] = 0.5*(Dxu[1 + j*(Ny+2)] + Dxu[1 + (j+1)*(Ny+2)]);
    }
    for (j = 1; j < Nx+1; j++){
        Dxu2[Ny+1 + j*(Ny+3)] = 0.5*(Dxu[Ny + j*(Ny+2)] + Dxu[Ny + (j+1)*(Ny+2)]);
    }
    ...
    for (j = 1; j < Nx+2; j++){
        for (i = 1; i < Ny+1; i++){
            gradabs[i + j*(Ny+2)] = sqrt(Dxu[i + j*(Ny+2)]*Dxu[i + j*(Ny+2)] + Dyu2[i + j*(Ny+2)]*Dyu2[i + j*(Ny+2)] + epsilon);
        }
    }
    for (j = 1; j < Nx+1; j++){
        for (i = 1; i < Ny+2; i++){
    ...
    for (j = 1; j < Nx+1; j++){
        for (i = 1; i < Ny+1; i++){
            gradabsmid[i + j*(Ny+2)] = sqrt(Dxu[i + j*(Ny+2)]*Dxu[i + j*(Ny+2)] + Dyu[i + j*(Ny+3)]*Dyu[i + j*(Ny+3)] + epsilon);
        }
    }
    ...
    for (j = 1; j < Nx+2; j++){
        for (i = 1; i < Ny+1; i++){
            Dx[i + j*(Ny+2)] = Dxu[i + j*(Ny+2)]/gradabs[i + j*(Ny+2)];
        }
    }
    for (j = 1; j < Nx+1; j++){
        for (i = 1; i < Ny+2; i++){
    ...
    for (i = 0; i < Ny; i++){
        for (j = 0; j < Nx; j++){
            ...
            for (k = 0; k < 3; k++){
                for (l = 0; l < 3; l++){
                    ind = l + 3*k;
                    u_l[ind] = u[i+l + (j+k)*(Ny+2)];
                    Dxul[ind] = Dxu[i+l + (j+k)*(Ny+2)];
                    Dxur[ind] = Dxu[i+l + (j+1+k)*(Ny+2)];
            ...
            fzeroFastMex(u_old+0.1, u_old,  u_l,  Dxul,  Dxuc, Dxur, Dxumt, Dxuml,
                    ...
                    E, K[i + j*Ny], g[i + j*Ny], i, j, Ny, Nx, dt,
            ...
            // ---------------------------------- update deriv approx
            if (j > 0){
                Dxu[i+1 + (j+1)*(Ny+2)] = u_l[4] - u_l[1];
            }
            if (j < Nx-1){
                Dxu[i+1 + (j+2)*(Ny+2)] = u_l[7] - u_l[4];
            }
            if (i > 0){
            ...
            for (k = 0; k < 3; k++){
                for (l = 1; l < 3; l++){
                    Dxu2[i+l + (j+k)*(Ny+3)] = 0.25*(Dxu[i+l-1 + (j+k)*(Ny+2)] + Dxu[i+l + (j+k)*(Ny+2)] + Dxu[i+l-1 + (j+1+k)*(Ny+2)] + Dxu[i+l + (j+k+1)*(Ny+2)]);
                }
            }
            for (k = 1; k < 3; k++){
                for (l = 0; l < 3; l++){
            ...
            if (i == 0){
                for (k = 0; k < 2; k++){
                    Dxu2[i+1 + (j+k)*(Ny+3)] = 0.5*(Dxu[i+1 + (j+k)*(Ny+2)] + Dxu[i+1 + (j+k+1)*(Ny+2)]);
                }
            }
            else if (i == Ny-1){
                for (k = 0; k < 2; k++){
                    Dxu2[i+2 + (j+k)*(Ny+3)] = 0.5*(Dxu[i+1 + (j+k)*(Ny+2)] + Dxu[i+1 + (j+k+1)*(Ny+2)]);
                }
            }
            if (j == 0){
                for (l = 0; l < 3; l++){
            ...
            for (k = 1; k < 3; k++){
                for (l = 0; l < 3; l++){
                    gradabs[i+l + (j+k)*(Ny+2)] = sqrt(Dxu[i+l + (j+k)*(Ny+2)]*Dxu[i+l + (j+k)*(Ny+2)] + Dyu2[i+l + (j+k)*(Ny+2)]*Dyu2[i+l + (j+k)*(Ny+2)] + epsilon);
                    Dx[i+l + (j+k)*(Ny+2)] = Dxu[i+l + (j+k)*(Ny+2)]/gradabs[i+l + (j+k)*(Ny+2)];
                }
            }
            for (k = 0; k < 3; k++){
                for (l = 1; l < 3; l++){
            ...
            for (k = 1; k < 3; k++){
                for (l = 1; l < 3; l++){
                    gradabsmid[i+l + (j+k)*(Ny+2)] = sqrt(Dxu[i+l + (j+k)*(Ny+2)]*Dxu[i+l + (j+k)*(Ny+2)] + Dyu[i+l + (j+k)*(Ny+3)]*Dyu[i+l + (j+k)*(Ny+3)] + epsilon);
                }
            }
        }
    }
